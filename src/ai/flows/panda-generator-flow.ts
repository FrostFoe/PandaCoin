'use server';
/**
 * @fileOverview An AI flow to generate whimsical panda names and backstories.
 *
 * - generatePandaDetails - A function that creates a panda name and backstory based on rarity.
 * - PandaGeneratorInput - The input type for the flow.
 * - PandaGeneratorOutput - The return type for the flow.
 */
...
export const PandaGeneratorInputSchema = z.object({
  rarity: z.enum(["Common", "Rare", "Ultra Rare"]).describe("The rarity of the panda, which should influence the tone and epicness of the name and backstory."),
});
export type PandaGeneratorInput = z.infer<typeof PandaGeneratorInputSchema>;

export const PandaGeneratorOutputSchema = z.object({
  name: z.string().describe("A whimsical, cute, or funny name for a panda. Should be short and memorable, around 2-3 words max."),
  backstory: z.string().describe("A short, creative, and family-friendly backstory for the panda (2-3 sentences). The story should match the panda's given rarity."),
});
...
export async function generatePandaDetails(input: PandaGeneratorInput): Promise<PandaGeneratorOutput> {
